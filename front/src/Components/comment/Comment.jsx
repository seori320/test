...
function Comment(props) {
	...
	const updateComment = async () => {
		...
		await axios.patch(`http://localhost:8080/post/${postId}/comment/update/${commentId}`, req, {headers: headers})
		.then((resp) => {
			console.log("[Comment.jsx] updateComment() success :D");
			console.log(resp.data);

			setShowModal(false);
			setAlertMessage("댓글을 성공적으로 수정했습니다 !");
	...
	const deleteComment = async () => {
		await axios.delete(`http://localhost:8080/post/${postId}}/comment/delete/${commentId}`, {headers: headers})
			.then((resp) => {
				console.log("[PostComment.jsx] deleteComment() success :D");
				console.log(resp.data);

				setAlertMessage("댓글을 성공적으로 삭제했습니다 :D");

				//삭제된 댓글 목록 다시 불러오기
				props.getCommentList(page);

			}).catch((err) => {
				console.log("[PostComment.jsx] deleteComment() error :<");
				console.log(err);
			});
	}
	...
	//if (comment.del == 0) {
		return (
			<>
				...
				<Modal show={alertMessage !== ""} onHide={() => setAlertMessage("")}>
					<Modal.Header closeButton>
						<Modal.Title>알림</Modal.Title>
					</Modal.Header>
					<Modal.Body>{alertMessage}</Modal.Body>
					<Modal.Footer>
						<Button variant="primary" onClick={() => setAlertMessage("")}>
							확인
						</Button>
					</Modal.Footer>
				</Modal>
